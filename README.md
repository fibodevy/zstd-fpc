...
Zstandard (RFC 8878) written in Pascal - encoder, decoder, and a dictionary trainer, written from scratch against the format specification. The streams are fully interchangeable with the reference libzstd library (verified both ways). Built for `{$mode unleashed}`.
